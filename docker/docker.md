...
一、可以通过`Dockerfile`来构建镜像，例如构建一个名为demo的镜像，可以执行`docker build -t demo .`; `.`是`Dockerfile` 文件所在位置。`Dockerfile`各参数含义如下:
...
- 用张图来解释
    ...
    ![src/dockerfile_parameter.png](src/dockerfile_parameter.png)
